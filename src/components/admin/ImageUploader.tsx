...
export default function ImageUploader({ images, onChange }: ImageUploaderProps) {
  ...
  const remove = (idx: number) => {
    const next = images.filter((_, i) => i !== idx).map((img, i) => ({
      ...img,
      orden: i,
      esPrincipal: i === 0 ? true : img.esPrincipal && i !== 0 ? false : img.esPrincipal,
    }));
    // Ensure at least one is principal
    if (next.length > 0 && !next.some((i) => i.esPrincipal)) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        ...
        className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-gray-700 hover:border-[#38bdf8] bg-black/40 rounded-lg px-6 py-10 cursor-pointer transition-colors duration-200 group"
      >
        <UploadIcon className="text-gray-600 group-hover:text-[#38bdf8] transition-colors" />
        <p className="text-sm text-gray-400 group-hover:text-white transition-colors">
          {uploading ? "Subiendo..." : "Arrastrá imágenes o hacé click para seleccionar"}
        </p>
        <p className="text-xs text-gray-600">PNG, JPG, WEBP — múltiples archivos</p>
        <input
          ref={inputRef}
          type="file"
          accept="image/*"
      ...
      {images.length > 0 && (
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
          {images.map((img, idx) => (
            <div key={img.url} className="relative group aspect-square bg-[#1a1a1a] rounded overflow-hidden border border-gray-800">
              <Image
                src={img.url}
                alt={`Imagen ${idx + 1}`}
                fill
