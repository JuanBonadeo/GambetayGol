...
export default function EditarProducto({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${params.id}`).then(r => r.json()).then(d => {
      if(d.data) {
        setForm({
          nombre: d.data.nombre, slug: d.data.slug, descripcion: d.data.descripcion || "", 
          categoria: d.data.categoria, precio: d.data.precio, clubId: d.data.clubId, 
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="bg-[#111] p-6 rounded-lg border border-gray-800 space-y-4">
        ...
        <div className="pt-4 flex gap-4">
          ...
          <Link href="/productos" className="px-4 py-2 text-gray-400 hover:text-white">Cancelar</Link>
        </div>
      </form>
    </div>
  );
