...
export default function EditarProducto({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${params.id}`).then(r => r.json()).then(d => {
      if (d.data) {
        setForm({
          nombre: d.data.nombre, slug: d.data.slug,
          descripcion: d.data.descripcion || "", categoria: d.data.categoria,
          precio: d.data.precio, clubId: d.data.clubId,
          destacado: d.data.destacado, activo: d.data.activo,
        });
        const existing: ProductImageEntry[] = (d.data.images ?? [])
          .sort((a: { orden: number }, b: { orden: number }) => a.orden - b.orden)
          .map((img: { url: string; orden: number; esPrincipal: boolean }) => ({
            url: img.url, orden: img.orden, esPrincipal: img.esPrincipal,
          }));
        setImages(existing);
      }
    });
